  ...
  return (
    <ParentSurface featured>
      ...
      <div className="relative">
        <p className="text-[11px] font-semibold tracking-[0.16em] text-gold-300 uppercase">Next Study Hall</p>
        {time ? (
          <p className="mt-5 font-display text-[2.85rem] font-semibold leading-[0.98] tracking-[-0.045em] text-white sm:text-[3.35rem]">
            {time}
          </p>
        ) : (
          <p className="mt-5 font-display text-[1.65rem] font-semibold tracking-[-0.03em] text-white sm:text-[1.85rem]">
            Time to confirm
          </p>
        )}
        <p className="mt-2.5 text-[15px] text-white/68">
          {day}
          {minutes ? ` · ${formatDuration(minutes)}` : ""}
        </p>
        <div className="mt-7">
          <p className="text-[1.35rem] font-medium tracking-[-0.02em] text-white">{child}</p>
          {guide ? (
        ...
        {join.state !== "join" && join.label ? (
          <p className="mt-5 text-sm font-medium text-gold-200">{join.label}</p>
        ) : null}
        {join.state === "join" ? (
          <div className="mt-7">
