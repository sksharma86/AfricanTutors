...
describe("Phase 5A — Daily access-window computation", () => {
  ...
  const open = new Date(now + 50 * 60_000).toISOString(); // opens in 50 min (start-10)
  ...
  it("student token expiration is tied to the normal session close", () => {
    ...
    assert.equal(w.roomNbf, Math.floor(new Date(open).getTime() / 1000) - 60, "room opens ~10 min before start");
  });
