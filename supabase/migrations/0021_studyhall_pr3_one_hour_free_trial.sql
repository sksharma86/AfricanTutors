-- Study Hall PR3: convert free trial from 30 minutes to 60 minutes (forward-only).
--
-- Business offer: "Your first 1-hour Study Hall session is free."
--
-- Changes (NEW free trials only):
--   * Free-trial duration authority: 30 → 60 minutes
--   * Free trial remains $0, no Stripe, no package minutes, no dollar credit
--   * One-free-trial-per-account protection is UNCHANGED
--
-- Historical safety:
--   * Existing free-trial bookings may be 30 minutes. The CHECK constraint is
--     widened to allow duration_minutes IN (30, 60) for is_free_trial rows so
--     historical rows remain valid. Authoritative RPCs reject non-60 for NEW
--     free trials.
--   * Paid list prices (session_list_price_cents) and package catalog from PR2
--     are UNCHANGED.
--   * Guide compensation / Stripe Connect / Auto Refill are UNCHANGED.
--
-- Do NOT apply this migration to production from the agent — apply after merge.

-- ---------------------------------------------------------------------------
-- CHECK constraint: allow historical 30-min free trials AND new 60-min free trials.
-- ---------------------------------------------------------------------------
alter table public.bookings
  drop constraint if exists bookings_free_trial_is_30;

alter table public.bookings
  drop constraint if exists bookings_free_trial_duration;

alter table public.bookings
  add constraint bookings_free_trial_duration
  check (not is_free_trial or duration_minutes in (30, 60));

-- ---------------------------------------------------------------------------
-- Redeclare booking_quote / book_session / create_booking from 0020 with only
-- the free-trial duration authority updated (30 → 60). Unrelated behavior is
-- preserved byte-for-byte aside from that intentional change (and booking_quote
-- now rejects free-trial quotes that are not 60 minutes, matching create/book).
-- ---------------------------------------------------------------------------
